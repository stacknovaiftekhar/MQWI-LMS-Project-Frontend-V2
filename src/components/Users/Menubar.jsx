import { FaUser, FaBook, FaMoneyBillAlt, FaSignOutAlt } from 'react-icons/fa';
...
const Menubar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Payment', path: 'payment', icon: <FaMoneyBillAlt /> },
          ...
          <ul className="navbar-nav ms-auto menu-list">
            {menuItems.map((item) => (
              <li key={item.name} className="nav-item">
                <NavLink to={`/dashboard/${item.path}`} className={`nav-link ${navLinkActive} ${item.class}`}>
